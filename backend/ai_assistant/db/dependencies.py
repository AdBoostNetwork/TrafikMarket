...
from ai_assistant.db.ai_session import make_ai_session
...
_ai_session_factory: async_sessionmaker | None = None
...
def init_ai_session_factory(ai_database_url: str) -> None:
    global _ai_session_factory
    _ai_session_factory = make_ai_session(ai_database_url)
    logger.info("ai_session_factory инициализирована")


async def get_ai_session() -> AsyncGenerator[AsyncSession, None]:
    if _ai_session_factory is None:
        raise RuntimeError("ai_session_factory не инициализирована")
    async with _ai_session_factory() as session:
        yield session


def get_ai_session_factory() -> async_sessionmaker:
    if _ai_session_factory is None:
        raise RuntimeError("ai_session_factory не инициализирована")
