...
_SQL_CPM_BY_FORMAT = text("""
    WITH cpm_rows AS (
      ...
      FROM tg_ads a
      JOIN announs an ON an.announ_id = a.ad_id
      JOIN tg_ads_prices p ON p.ad_id = a.ad_id
      WHERE an.status = 'active' AND a.topic = :topic_id
      UNION ALL
      SELECT p.format, p.price / NULLIF(a.cover_count, 0) * 1000
      FROM max_ads a
      JOIN announs an ON an.announ_id = a.ad_id
      JOIN max_ads_prices p ON p.ad_id = a.ad_id
      WHERE an.status = 'active' AND a.topic = :topic_id
      UNION ALL
      SELECT p.format, p.price / NULLIF(a.cover_count, 0) * 1000
      FROM tg_net_ads a
      JOIN announs an ON an.announ_id = a.ad_id
      JOIN tg_net_ads_prices p ON p.ad_id = a.ad_id
      ...
      FROM max_net_ads a
      JOIN announs an ON an.announ_id = a.ad_id
      JOIN max_net_ads_prices p ON p.ad_id = a.ad_id
      JOIN max_net_ads_topics t ON t.ad_id = a.ad_id
      WHERE an.status = 'active' AND t.topic_id = :topic_id
    )
...
class AvgAdCpm(Tool):
    name = "avg_ad_cpm"
    description = (
        "Средний CPM (цена за 1000 охвата) рекламных размещений в заданной тематике — по всем "
        "форматам темы (1/24, 2/48, repost и т.п.), в USDT и рублях (курс из БД). Считается по "
        "активным объявлениям четырёх рекламных типов: одиночные и сети, Telegram и MAX. "
        "Сторис, каналы и трафик сюда не входят."
    )
    Params = _Params
    ...
    async def execute(self, ctx: ToolContext, params: _Params) -> dict:
        ...
        logger.info(
            "средний CPM рекламы посчитан | topic=%s | user_id=%s | rate=%s | formats=%s",
