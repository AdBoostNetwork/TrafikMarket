...
Создаёт роль `ai_readonly` (идемпотентно, через проверку `pg_roles`) с правом только `SELECT`: `CONNECT` на БД, `USAGE` на схему `public`, `SELECT` на все текущие таблицы схемы `public`, а также `ALTER DEFAULT PRIVILEGES`, чтобы `SELECT` автоматически распространялся на таблицы будущих миграций. Никаких прав на INSERT/UPDATE/DELETE/DDL роль не получает — запись для неё структурно невозможна на уровне БД. Пароль роли берётся из переменной окружения `AI_DB_PASSWORD` (миграция падает, если переменная пуста или не задана) и не хранится в коде миграции. Под этой ролью ходит аналитика контейнера `ai-assistant` к основной БД (`MAIN_DATABASE_URL`).
...
| `created_at` | `timestamp with time zone` | да | `—` | Дата и время создания транзакции |
...
| `announ_type` | `text` | да | `—` | Тип объявления сделки (денормализовано из `announ_types.type_name` в момент записи) |
| `amount` | `numeric(10,2)` | да | `—` | Сумма операции |
